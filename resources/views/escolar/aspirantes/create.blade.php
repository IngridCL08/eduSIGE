...
@section('content')
<div class="max-w-4xl">
<form method="POST" action="{{ route('escolar.aspirantes.store') }}" class="space-y-5">
    @csrf
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div>
                ...
                <select name="sexo" class="form-select @error('sexo') border-danger @enderror" required>
                    ...
                    <option value="O" {{ old('sexo') === 'O' ? 'selected' : '' }}>Otro / Prefiero no decir</option>
                </select>
            ...
            <div>
                ...
                <input name="curp" value="{{ old('curp') }}" maxlength="18"
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input type="tel" name="telefono" value="{{ old('telefono') }}"
                       class="form-input @error('telefono') border-danger @enderror" placeholder="10 dígitos">
            ...
            <div>
                ...
                <input name="colonia" value="{{ old('colonia') }}" class="form-input">
            </div>
            <div>
                <label class="form-label">Municipio / Ciudad</label>
                <input name="municipio" value="{{ old('municipio') }}" class="form-input">
            </div>
            <div>
                <label class="form-label">Estado</label>
                <input name="estado" value="{{ old('estado') }}" class="form-input">
            </div>
            <div>
                <label class="form-label">Código Postal</label>
                <input name="cp" value="{{ old('cp') }}" class="form-input" placeholder="00000" maxlength="10">
            </div>
        </div>
    </div>
    ...
    <div class="card">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input name="bachillerato" value="{{ old('bachillerato') }}"
            ...
            <div>
                ...
                <input type="number" name="anio_egreso" value="{{ old('anio_egreso') }}"
                       class="form-input" placeholder="{{ date('Y') }}" min="1980" max="{{ date('Y') }}">
            </div>
        </div>
    </div>

    <div class="flex justify-end gap-3">
        <a href="{{ route('escolar.aspirantes.index') }}" class="btn-secondary">Cancelar</a>
